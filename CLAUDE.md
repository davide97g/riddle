...
- pen samples out on stdout as `P <t_ms> <x> <y> <pressure>` / `U <t_ms>`, plus `K PEN|RUBBER` when the pen presents an end and `T <x> <y>` for a finger
...
- `riddle.py` — the loop. Accumulates strokes, and when the pen has been *lifted* and idle ≥ `RIDDLE_PAUSE_MS`, fires `answer()` — but only if something was actually **written**. A pause is not a question on its own: `finish_stroke` throws away presses (travel < `TAP_TRAVEL`, i.e. a toolbar tap, a poke to wake the page, a blot), anything wholly inside the left toolbar strip (`TOOLBAR_X`), and eraser passes, which instead delete the pending strokes they cross; `question()` then requires `MIN_INK` of travel spanning `MIN_SPAN`. Finger touches only push the clock back, since handling the tablet is not writing. The nib/eraser distinction comes from the agent's `K PEN|RUBBER` line (real `BTN_TOOL_*`); a *UI-selected* eraser used with the nib is indistinguishable from writing at the evdev layer and still reads as ink. Pen-down is tracked explicitly because a resting nib emits nothing (dropped duplicate coordinates), so silence alone is not a pause. `answer()` starts the model on a thread and erases the page concurrently, so the ink starts fading immediately; if the model returns nothing, the original strokes are redrawn rather than leaving a blank page.
