...
| `riddle.mind` | who answers. `openai` (one http call per turn, with the page attached to it), `persona` (the words, which outlived the two models that said them before), `memory` (the lines that survive a reset) |
