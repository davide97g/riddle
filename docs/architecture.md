...
| `riddle.mind` | `llm` (the persona, and the claude subprocess), `memory` (the lines that survive a reset) |
...
| top level | `paths`, `config`, `consent`, `process`, `tailnet`, `probe`, `doctor`, `cli` |
