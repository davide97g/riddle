...
def layout(
    ...
    """Lay text out centred on center_x, first line sitting on baseline."""
    font = font or Font()
    scale = height / _UNITS_PER_EM
    strokes: list[Polyline] = []

    for row, line in enumerate(wrap(text, font, scale, max_width)):
        pen_x = center_x - font.width(line, scale) / 2
        pen_y = baseline + row * height * line_gap
        for ch in line:
            strokes.extend(font.glyph(ch, pen_x, pen_y, scale))
            pen_x += font.advance(ch, scale)
    return strokes
